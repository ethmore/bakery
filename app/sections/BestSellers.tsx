...
function BestSellers() {
    return (
      <div className="relative w-full h-[700px] mt-40">
          <div className="absolute w-full h-[600px] overflow-hidden -z-10">
              <Image src={`./general/yeh-xintong-go3DT3PpIw4-unsplash.webp`} alt="En çok satanlar arkaplan" fill className="filter blur-[3px] brightness-50 scale-105 object-cover"/>
          </div>
  ...
  function BestSellerItem({src, alt, description}:BestSellerItemProps) {
      return (
          <Link href="/urunlerimiz" className="select-none relative min-h-[450px] min-w-[300px] xl:min-w-[200px] flex overflow-hidden shadow-xl shadow-gray-500 rounded-2xl group hover:scale-105 duration-300 transition-transform ">
              <Image src={src} alt={alt} fill className="object-cover rounded-2xl"/>
              <div className="absolute left-0 -bottom-20 backdrop-blur-xl backdrop-brightness-50 w-full p-4 text-xl text-center text-white group-hover:bottom-0 transition-all duration-300">{description}</div>
          </Link>
      )
  }
